@extends('layouts.app')
@section('title', 'Edit Booking Condition')
@section('content')
<div class="max-w-2xl mx-auto">
    ...
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Edit Booking Condition</h1>

    <form method="POST" action="{{ route('booking-conditions.update', $bookingCondition->id) }}" class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input 
                ...
                name="title" 
                id="title" 
                value="{{ old('title', $bookingCondition->title) }}" 
        ...
        <div>
            ...
            >{{ old('description', $bookingCondition->description) }}</textarea>
            @error('description')
                <span class="text-sm text-red-600 mt-1">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input 
                ...
                name="sort_order" 
                id="sort_order" 
                value="{{ old('sort_order', $bookingCondition->sort_order) }}" 
                placeholder="0"
                min="0"
        ...
        <div class="flex items-center gap-3">
            <input 
                ...
                id="is_active" 
                value="1"
                {{ old('is_active', $bookingCondition->is_active) ? 'checked' : '' }}
