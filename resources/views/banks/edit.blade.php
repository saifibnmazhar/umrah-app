...
<div class="max-w-2xl mx-auto">
    ...
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Edit Bank</h1>

    <form method="POST" action="{{ route('banks.update', $bank->id) }}" class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input 
                ...
                name="description" 
                id="description" 
                value="{{ old('description', $bank->description) }}" 
        ...
        <div>
            ...
            >
                ...
                <option value="SAR" @selected(old('currency', $bank->currency?->value ?? '') == 'SAR')>SAR</option>
                <option value="BDT" @selected(old('currency', $bank->currency?->value ?? '') == 'BDT')>BDT</option>
            </select>
            @error('currency')
                <span class="text-sm text-red-600 mt-1">{{ $message }}</span>
            @enderror
