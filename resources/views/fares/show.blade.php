...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('fare.admin') }}" class="px-4 py-2 border border-slate-300 text-slate-700 rounded-md hover:bg-slate-50 transition text-sm font-medium">
            Back to List
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-1">
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
                ...
                <div class="space-y-2">
                    <a href="{{ route('fare.admin') }}" class="block w-full text-center px-4 py-2 bg-slate-700 text-white rounded-md hover:bg-slate-600 transition text-sm font-medium">
                        Edit
                    </a>
                    <form method="POST" action="{{ route('fare.admin.fare.destroy', $ticketFare->id) }}" onsubmit="return confirm('Are you sure you want to delete this ticket fare?')">
                        @csrf
                        @method('DELETE')
