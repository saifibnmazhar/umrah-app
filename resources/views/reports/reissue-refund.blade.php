@extends('layouts.app')
@section('title', 'Re-Issue & Refund Report')
@section('content')
<div class="max-w-3xl mx-auto" x-data="{ date_from: '', date_to: '', type: '' }">
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Re-Issue & Refund Report</h1>
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select x-model="type" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent">
        ...
        <table class="min-w-full bg-white border border-slate-200 rounded-lg shadow-sm">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Type</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Amount</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Reason</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-200">
                <tr>
                    ...
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $type ?? 'Re-Issue' }}</td>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $amount ?? '5000' }}</td>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $reason ?? 'Schedule Change' }}</td>
                </tr>
            </tbody>
        </table>
    </div>
