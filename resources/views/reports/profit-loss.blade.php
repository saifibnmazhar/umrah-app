...
<div class="max-w-3xl mx-auto" x-data="{ date_from: '', date_to: '', agent: '' }">
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Profit/Loss Report</h1>
        ...
        <table class="min-w-full bg-white border border-slate-200 rounded-lg shadow-sm">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Cost</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Profit</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Loss</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-200">
                <tr>
                    ...
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $route ?? 'DXB-JFK' }}</td>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $ticket_fare ?? '50000' }}</td>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $cost ?? '45000' }}</td>
                    <td class="px-3 py-2 text-sm text-green-600">{{ $profit ?? '5000' }}</td>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $loss ?? '-' }}</td>
                </tr>
            </tbody>
        </table>
    </div>
