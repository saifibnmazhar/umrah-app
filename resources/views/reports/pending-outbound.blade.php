...
<div class="max-w-3xl mx-auto" x-data="{ date_from: '', date_to: '', status: '' }">
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Pending Outbound Ticket Report</h1>
        ...
        <table class="min-w-full bg-white border border-slate-200 rounded-lg shadow-sm">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-slate-600 uppercase">Flight Date</th>
            ...
            <tbody class="divide-y divide-slate-200">
                <tr>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $booking_date ?? '2024-01-01' }}</td>
                    ...
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $flight_date ?? '2024-02-01' }}</td>
                    <td class="px-3 py-2 text-sm text-slate-700">{{ $route ?? 'DXB-JFK' }}</td>
                    <td class="px-3 py-2 text-sm text-yellow-600">{{ $status ?? 'Pending' }}</td>
                </tr>
            </tbody>
        </table>
    </div>
