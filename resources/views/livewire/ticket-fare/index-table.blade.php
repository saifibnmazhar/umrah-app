    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="p-4 flex flex-wrap gap-4 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" wire:model.live="search" placeholder="Search by airline..." class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm">
            </div>
            ...
            <div>
                <button wire:click="$reset('search', 'airlineFilter', 'ticketTypeFilter', 'statusFilter')" type="button" class="px-4 py-2 border border-slate-300 text-slate-700 rounded-md hover:bg-slate-50 transition text-sm font-medium">
                    Clear
                </button>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('ticket-fares.show', $fare->id) }}" class="text-xs text-slate-600 hover:text-slate-800 font-medium">View</a>
                                    @if($fare->is_locked)
                                    ...
                                    @else
                                        <a href="{{ route('ticket-fares.edit', $fare->id) }}" class="text-xs text-slate-600 hover:text-slate-800 font-medium">Edit</a>
