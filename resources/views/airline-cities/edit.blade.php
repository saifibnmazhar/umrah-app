@extends('layouts.app')
@section('title', 'Edit Airline City')
@section('content')
<div class="max-w-2xl mx-auto">
    ...
    <h1 class="text-2xl font-bold text-slate-800 mb-6">Edit Airline City</h1>

    <form method="POST" action="{{ route('airline-cities.update', $airlineCity->id) }}" class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="airline_id" id="airline_id" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent @error('airline_id') border-red-500 @enderror">
                ...
                @foreach($airlines as $airline)
                    <option value="{{ $airline->id }}" {{ (old('airline_id') ?? $airlineCity->airline_id) == $airline->id ? 'selected' : '' }}>
                        {{ $airline->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="city_code_id" id="city_code_id" class="w-full px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent @error('city_code_id') border-red-500 @enderror">
                ...
                @foreach($cityCodes as $cityCode)
                    <option value="{{ $cityCode->id }}" {{ (old('city_code_id') ?? $airlineCity->city_code_id) == $cityCode->id ? 'selected' : '' }}>
