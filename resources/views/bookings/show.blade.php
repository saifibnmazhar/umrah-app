    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="text-lg text-slate-800">{{ $booking->discount_type?->value ? ucfirst(str_replace('_', ' ', $booking->discount_type->value)) : 'None' }}</p>
            </div>
            <div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-200">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 text-slate-700">{{ ucfirst($passenger->passenger_type?->value ?? 'None') }}</td>
                        <td class="px-3 py-2 text-slate-700">{{ $passenger->date_of_birth }}</td>
                        <td class="px-3 py-2 text-slate-700">{{ $passenger->service_required?->value ?? 'None' }}</td>
                        <td class="px-3 py-2 text-slate-700">{{ $passenger->ticket_status?->value ?? 'None' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
