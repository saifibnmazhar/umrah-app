...
```bash
docker compose up -d       # Start app + MySQL (port 8080)
```
...
1. **test-php** — Sets up PHP 8.3, starts MySQL 8.0 service, runs migrations + PHPUnit
...
```bash
docker compose up -d          # Build + start app + MySQL
...
2. **PHP 8.3-fpm-alpine** — installs PHP extensions (pdo_mysql, mysqli, intl, mbstring, zip, redis), Composer deps, Nginx, Supervisord
