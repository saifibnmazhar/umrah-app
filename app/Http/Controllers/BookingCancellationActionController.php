...
class BookingCancellationActionController extends Controller
{
    ...
    public function reportData(Request $request)
    {
        ...
        $data = $paginated->getCollection()->map(fn($cb) => [
            ...
            'customer_mobile' => $cb->booking?->customer?->mobile ?? '-',
            ...
            'refund_method' => $cb->refundPayment?->payment_method?->value ?? '-',
            'refund_remarks' => $cb->refundPayment?->remarks ?? '-',
            'cancel_date' => $cb->created_at->format('Y-m-d H:i'),
            'refund_date' => $cb->refundVoucher?->created_at?->format('Y-m-d H:i') ?? '-',
